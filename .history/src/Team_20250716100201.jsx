...
const Team = () => {
  return (
    <section id="team" className="py-16 px-6 bg-white dark:bg-gray-900">
      ...
      {/* Redempta Kanja full-width */}
      <div className="relative group bg-gray-50 dark:bg-gray-800 rounded-lg shadow-md p-6 text-center hover:shadow-xl transition max-w-3xl mx-auto mb-12">
        <img
          src={redempta.image}
          alt={redempta.name}
        ...
        {/* Hover Details */}
        <div className="absolute left-1/2 transform -translate-x-1/2 bottom-6 opacity-0 group-hover:opacity-100 transition duration-300 bg-white dark:bg-gray-700 text-gray-800 dark:text-white px-4 py-2 rounded shadow text-xs w-11/12 max-w-xs">
          <p className="truncate">{redempta.email}</p>
          <p>{redempta.phone}</p>
        </div>
      </div>
      ...
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto">
        {teamMembers.map((member, index) => (
          <div
            key={index}
            className="relative group bg-gray-50 dark:bg-gray-800 rounded-lg shadow-md p-6 text-center hover:shadow-xl transition"
          >
            <img
              src={member.image}
              alt={member.name}
