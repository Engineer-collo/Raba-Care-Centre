...
export default function Milestones() {
  return (
    <div className="bg-white p-8 rounded-2xl shadow-lg border-l-8 border-red-900">
      <h2 className="text-3xl font-bold text-red-900 mb-6 text-center">
        Rabacare Journey
      </h2>

      <div className="relative border-l-4 border-amber-500 pl-6 space-y-8">
        {milestones.map((m, i) => (
          <motion.div
            key={i}
            className="relative"
          ...
          >
            <span className="absolute -left-8 flex items-center justify-center w-10 h-10 bg-red-900 text-white rounded-full shadow">
              {m.icon}
            </span>
            <div className="bg-gray-50 p-4 rounded-xl shadow">
              <h3 className="text-xl font-semibold text-amber-600">{m.year} — {m.title}</h3>
